...
roberta_model = RobertaModel.from_pretrained(model_name_or_path=pretrained_model_dir, checkpoint_file="checkpoint_best.pt")
roberta_model.eval()
print(roberta_model)
for count, batch_df in enumerate(batched_data):
    for tokens in batch_df.itertuples(index=False):
        ...
        tokens = roberta_model.encode(tokens[from_col], tokens[to_col])
        print(tokens)
        decoded = roberta_model.decode(tokens)
        print(decoded)
